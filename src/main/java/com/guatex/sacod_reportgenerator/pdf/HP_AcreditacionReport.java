...
public class HP_AcreditacionReport extends Pdf_Utils {
	...
	public HP_AcreditacionReport(String nombreArchivo) throws FileNotFoundException, DocumentException, Exception {
		try {
	...
	public PdfPTable getResumen(Tabla tbl, int width) throws Exception {
		float widths[] = { 1, 1,1, 0.7f, 1, 2, 0.5f, 1, 0.5f, 1, 0.5f, 1,1.3f };
	...
	public PdfPTable getInfo(Cliente cliente) throws Exception {
		...
		table.addCell(cInfo);
		/*
		 * cInfo = nuevaCelda("ACREDITAMIENTO #: " + idACH, Element.ALIGN_CENTER, 2,
		 * normalFontTitleMedium); cInfo.setBorder(2); cInfo.setBorderWidth(1.35f);
		 */
		...
		/*
		 * table.addCell(cSpace); table.addCell(cSpace); table.addCell(cInfo);
		 */
		return table;
	}
